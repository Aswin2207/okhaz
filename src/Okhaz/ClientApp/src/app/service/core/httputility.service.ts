...
export class HttpUtilityService {
      ...
      try {
        ...
        this.http
          .post(
            url,
            JSON.stringify(data),
            option ? option : { headers: this.getHeaders() }
          )
          .subscribe(
            (res) => {
              resolve(res);
  ...
  put(url: string, data?: any, option?: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        url = environment.serverAddress + "/" + url;
        this.http
          .put(url, data, option ? option : { headers: this.getHeaders() })
          .subscribe(
            (res) => {
              this.helper.stopWorkProgressBar();
              resolve(res);
  ...
  uploadFileWithProgress(
    ...
    let observable = new Observable<{}>((observer) => {
      ...
      try {
        let sub = this.http.request(
          new HttpRequest(method, environment.serverAddress + "/" + url, data, {
            reportProgress: true,
            headers: headers ? headers : this.getHeaders(),
          })
        );
        this.uploadObsr = sub.subscribe(
          (event) => {
            switch (event.type) {
  ...
  multipeApiCalls(url: any[], options?: any, temp?: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      url.forEach((ele) => {
        ...
        if (ele["Type"] == "GET") {
          subURL = this.http
            .get(environment.serverAddress + "/" + ele.URL)
            .toPromise();
        } else if (ele["Type"] == "POST") {
          subURL = this.http
            .post(environment.serverAddress + "/" + ele.URL, ele.Data)
            .toPromise();
        } else if (ele["Type"] == "PUT") {
          subURL = this.http
            .put(environment.serverAddress + "/" + ele.URL, ele.Data)
            .toPromise();
        }
        arr.push(subURL);
      });
