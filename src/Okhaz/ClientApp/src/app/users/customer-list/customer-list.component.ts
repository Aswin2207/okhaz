...
export class CustomerListComponent implements OnInit {
  ...
  getDatFromServer(gridModel: any) {
    ...
    // }
    //   , (error: any) => { })
    this.http.get(AppconstantsService.UsersAPIS.GetCustomerDetailsbyBranchID).then((data: any) => {
      var updatedData = [];
      data.forEach(element => {
        updatedData.push({
          user: {
            displayName: element.custName,
            image: element.AvatarLocation,
            id: element.custID,
            city: element.City,
            country: element.Country,
            email: element.cusEmail,
            address: element.cusAdd1,
            isNew: false
          },
          displayName: element.custName,
          email: element.cusEmail,
  ...
  onAnyAction(e: any) {
    console.log('MyAction:',e);
    switch (e.action) {
      case 'rowSelected':
        this.http.detailPageData = e.row;
        this.route.navigate(["/user/customer/detail"]);
        break;
      case "click":
