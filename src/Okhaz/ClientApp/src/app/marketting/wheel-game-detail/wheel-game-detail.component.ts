...
import { CodeValueModel, WheelModel, WheelConfigModel, WheelActionModel, ActionItems } from '../../models/wheel.model';
...
export class WheelGameDetailComponent implements OnInit, OnDestroy {
  ...
  constructor(public http: HttpUtilityService, private route: Router, private helper: HelperService, private modalService: NgbModal) {
  }
  ...
  ngOnInit(): void {
    ...
    this.mainImgPath = this.formData.ImagePath;
    if (this.http.detailPageData) {
      this.row = this.http.detailPageData;
      console.log('row', this.row);
      this.deviceList = [
        { code: 'Android', value: this.row.IsActiveOnAndriod == 1 },
        { code: 'iPhone', value: this.row.IsActiveOniOS == 1 },
        { code: 'Website', value: this.row.IsActiveOnWeb == 1 }
      ];
      this.getWheelSlices();
    }
    else {
      this.route.navigate(["/marketting/wheel-game"]);
    }
  }
  ...
  onFileUploaded(event: any) {
    ...
    fileReader.onload = (e) => {
      if (fileReader.result) {
        this.formData.ImagePath = fileReader.result;
        this.selectedImage = fileReader.result;
      }
    }
    fileReader.readAsDataURL(file[0]);
    this.formData.ImagePath = file[0];
  }
  ...
  toggle(e: any, isAll: boolean, device: CodeValueModel) {
    ...
    var updateWheelMaster = {
      WGlID: parseInt(this.row.WGlID),
      BranchID: this.branchInfo.id,
      WGTitle: this.row.WGTitle,
      WGTextSize: parseInt(this.row.WGTextSize),
      SliceRepeat: parseInt(this.row.SliceRepeat),
      SpinTime: parseInt(this.row.SpinTime),
      FairMode: parseInt(this.row.FairMode),
      LastUpdateDate: new Date(),
      UserID: this.userInfo.id,
      WinnerID: this.row.WinnerID,
      WGScheduleTime: this.getServerDate(this.row.WGScheduleTime),
      WGDescription: this.row.WGDescription,
      MessageAfterWin: this.row.MessageAfterWin,
      WGStatus: 1,
      LastUse: this.getServerDate(this.row.LastUse),
      IsActiveOnAndriod: this.deviceList[0].value==true?1:0,
      IsActiveOniOS: this.deviceList[1].value==true?1:0,
      IsActiveOnWeb:this.deviceList[2].value==true?1:0
    }
    this.http.post(AppconstantsService.WheelsAPIS.UpdateWheelGameMaster, updateWheelMaster).then((data: any) => { })
  }
  ...
  getServerDate(date: any) {
    ...
    var trimTime = date.split(" ")[1];
    var dates = trimDate.split("-");
    var times = trimTime.split(":");
    var updatedDate = new Date();
    updatedDate.setFullYear(dates[0]);
    updatedDate.setMonth(dates[1]);
    updatedDate.setDate(dates[2]);
    updatedDate.setHours(times[0]);
    updatedDate.setMinutes(times[1]);
    updatedDate.setSeconds(times[2]);
    return updatedDate;
  }
  ...
  apply() {
    const index: number = this.dataSource.findIndex(x => x.WGSID == this.formData.WGSID);
    if (index !== -1 && this.isEdit) {
      this.dataSource[index] = JSON.parse(JSON.stringify(this.formData));
      var updateWheelSliceJson = {
        WGSID: parseInt(this.dataSource[index].WGSID),
        WGlID: parseInt(this.row.WGlID),
        WGSText: this.dataSource[index].WGSText,
        WGSTextColorCode: this.dataSource[index].WGSTextColorCode,
        WGSBackGroudColorCode: this.dataSource[index].WGSBackGroudColorCode,
        ImagePath: this.dataSource[index].ImagePath,
        UserID: this.userInfo.id,
        WGSstatus: 1
      };
      this.http.post(AppconstantsService.WheelsAPIS.UpdateWheelGameSlice, updateWheelSliceJson).then((data: any) => {
        this.getWheelSlices();
      }, (error: any) => { })
      ...
      this.formData.WGSID = lastItem && lastItem.id ? (parseInt(lastItem.id) + 1).toString() : '1';
      var createWheelSliceJson = {
        WGlID: parseInt(this.row.WGlID),
        WGSText: this.formData.WGSText,
        WGSTextColorCode: this.formData.WGSTextColorCode,
        WGSBackGroudColorCode: this.formData.WGSBackGroudColorCode,
        ImagePath: this.formData.ImagePath,
        UserID: this.userInfo.id,
        WGSstatus: 1
      };
      this.http.post(AppconstantsService.WheelsAPIS.CreateWheelGameSlice, createWheelSliceJson).then((data: any) => {
        this.getWheelSlices();
      }, (error: any) => { })
      this.action = { index: this.dataSource.length - 1, action: ActionItems.ADD, param: [this.formData] };
    }
    this.dataSource = this.dataSource.filter(x => x.WGSID);
    this.closeModel();
  }
  ...
  delete(id: string) {
    this.action = { index: this.dataSource.findIndex(x => x.WGSID == id), action: ActionItems.DELETE, param: [this.formData] };
    this.dataSource = this.dataSource.filter(x => x.WGSID !== id);
    this.closeModel();
  }
  ...
  setTableData(data: any, gridModel: any) {
    this.dataSource = data.filter(x => x.WGSID);
    // this.wheelConfig.totalRows = data.length;
  }

  getDatFromServer(gridModel: any) {
    this.OrderDetailsConfig.currentPageSize = gridModel.limit;
    this.http.get(AppconstantsService.WheelsAPIS.GetWheelSliceByWGlID + this.row.WGlID).then((data: any) => {
      this.setTableData(data, gridModel);
    }, (error: any) => { })
  }

  cloneWheel(wheelID) {
    if (confirm('Are you sure you want to clone this wheel?')) {
      // Save it!
      this.http.get(AppconstantsService.WheelsAPIS.CloneWheelGameMaster + wheelID).then((data: any) => {
        console.log(data);
      }, (error: any) => { })
    } 
  }
